...
Data_process::Data_process(string vis_namee, int indexx, string fd_namee, string data_typee, string size_sufxx, double err_ampdd , double err_phasee, string main_fd_namee,int creat_main_fd_ctrll)
{
	vis_name = vis_namee;
	index = indexx;
	fd_name = fd_namee;
...
void Data_process::rec_info(ofstream & out)
{
	...
	out << "The data are stored in this text." << endl << "This is the " << vis_name << " case."
		...
		<< "The scheme option is " << q_type << endl << "The flux option is " << flux_type << endl;
	wt_string += "The parameters are:\n\n";

	para(0) = N; para(1) = x_low; para(2) = x_sup; para(3) = M; para(4) = a; para(5) = T; para(6) = nu; para(7) = dx; para(8) = dt; para(9) = N_itr;
	para(10) = M_fac; para(11) = NT; para(12) = xi; para(13) = d; para(14) = err; para(15) = err_ampd; para(16) = err_phase;
	name_para(0) = "N"; name_para(1) = "x_low"; name_para(2) = "x_sup"; name_para(3) = "M"; name_para(4) = "a"; name_para(5) = "T";
	name_para(6) = "nu"; name_para(7) = "dx"; name_para(8) = "dt"; name_para(9) = "N_itr";
	name_para(10) = "M_fac"; name_para(11) = "NT"; name_para(12) = "xi"; name_para(13) = "d"; name_para(14) = "err"; name_para(15) = "err_ampd"; name_para(16) = "err_phase";
...
void Data_process::eva_Fourier_err(void)
{
	if (bnd_type == "PD")
	{
		err_ampd = un.max()/uf_exact.max() ;
		err_phase = x(un.index_max())-x(uf_exact.index_max()) ;
		cout << "q_type= " << q_type << endl << "nu=" << nu << endl;
		cout << "un=" << un << endl << "uf_exact=" << uf_exact << endl;
		cout << "un.index_max" << un.index_max() << endl << "uf_exact.index_max" << uf_exact.index_max() << endl;
		cout << "un.index_max" << maxindex(un) << endl << "uf_exact.index_max" << maxindex(uf_exact) << endl;
	}
}
